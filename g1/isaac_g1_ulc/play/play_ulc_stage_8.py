#!/usr/bin/env python3
"""
ULC G1 Stage 8 Play Script
============================
Evaluation script for Stage 8: position perfection + orientation control.

Two-phase curriculum:
  Phase 1 (Levels 0-5): Position perfection - tighter thresholds, faster reaching
  Phase 2 (Levels 6-10): Orientation control - palm_down then variable orientations
...
Usage:
    ...
    ./isaaclab.bat -p play_ulc_stage8.py \
...
parser = argparse.ArgumentParser(description="ULC G1 Stage 8 Play - Position Perfection + Orientation")
...
class FrameRecorder:
    ...
    def finalize_video(self, output_name: str = "stage8_reaching_demo.mp4"):
        import subprocess
        import shutil
...
# ============================================================================
# TRAINING CURRICULUM LEVELS (must match train_ulc_stage_8.py EXACTLY)
...
class ArmActor(nn.Module):
    """55 obs -> 5 actions (Stage 8: position perfection + orientation obs)"""
...
def main():
    ...
    print(f"\n{'=' * 70}")
    if args.record:
        print("ULC G1 STAGE 8 - VIDEO RECORDING")
    else:
        print("ULC G1 STAGE 8 - POSITION PERFECTION + ORIENTATION PLAY")
    print(f"{'=' * 70}")
    ...
    if args.record:
        ...
        record_dir = os.path.join(os.getcwd(), "recordings", f"stage8_{timestamp}")
    ...
    # Finalize video
    if recorder:
        video_path = recorder.finalize_video(f"g1_stage8_reaching_{args.mode}.mp4")
        if video_path:
            print(f"\n[VIDEO] LinkedIn-ready video: {video_path}")

    total_attempts = env.validated_reaches + env.timed_out_targets
    print(f"\n{'=' * 70}")
    print("STAGE 8 PLAY COMPLETE")
    print(f"{'=' * 70}")
