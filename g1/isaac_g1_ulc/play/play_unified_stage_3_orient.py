"""
Unified Stage 3: Orientation Fine-Tune — Play/Evaluation (29DoF)
================================================================
Dual Actor-Critic: Loco (FROZEN, 66->15) + Arm (39->7).
Both policies deterministic during evaluation.
Loads Stage 3 checkpoints (orient fine-tune with critic reset).
...
ARM_ACTION_SCALE = 2.0  # Override cfg
...
# Stage 3 curriculum (6 levels) — MUST match train_unified_stage_3_orient.py
CURRICULUM = [
    ...
    {"pos_threshold": 0.04, "min_displacement": 0.06, "max_reach_steps": 200, "use_orientation": True, "workspace_radius": (0.18, 0.55), "orient_threshold": 2.5},
    ...
    {"pos_threshold": 0.04, "min_displacement": 0.06, "max_reach_steps": 200, "use_orientation": True, "workspace_radius": (0.18, 0.55), "orient_threshold": 1.2},
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 3 Orient Fine-Tune — Play/Eval (29DoF)")
    ...
    parser.add_argument("--reach_threshold", type=float, default=None,
                        help="Override pos_threshold (meters)")
...
# ============================================================================
# ENVIRONMENT (same scene as Stage 2 play)
# ============================================================================
...
def create_env(num_envs, device):
    ...
    class SceneCfg(InteractiveSceneCfg):
        dome_light = AssetBaseCfg(
            ...
            spawn=sim_utils.DomeLightCfg(intensity=1500.0, color=(0.95, 0.95, 1.0)),
        )
        distant_light = AssetBaseCfg(
            prim_path="/World/DistantLight",
            spawn=sim_utils.DistantLightCfg(intensity=3000.0, color=(1.0, 0.98, 0.90)),
    ...
    class PlayEnv(DirectRLEnv):
        ...
        def _sample_targets(self, env_ids):
            ...
            ws_r = self.play_curriculum.get("workspace_radius", (0.18, 0.55))
            ...
            min_dist = self.play_curriculum.get("min_displacement", 0.06)
        ...
        def _pre_physics_step(self, combined_act):
            ...
            tgt[:, self.loco_idx[12]].clamp_(-0.15, 0.15)
            tgt[:, self.loco_idx[13]].clamp_(-0.15, 0.15)
            tgt[:, self.loco_idx[14]].clamp_(-0.2, 0.2)
            for hy_idx in self.hip_yaw_loco_idx:
                tgt[:, self.loco_idx[hy_idx]].clamp_(-0.3, 0.3)
            arm_act = arm_act.clamp(-1.5, 1.5)
...
def main():
    ...
    reach_distances = []
    reach_displacements = []
    reach_orient_errs = []  # Stage 3: track orient error at reach
    ...
    orient_str = "NO_ORIENT" if args_cli.no_orient else f"ORIENT_CHECK(thr={orient_thresh:.1f})"
    print(f"\n{'='*80}")
    print(f"STAGE 3 PLAY: {mode_str} | Level {ckpt_level} | {orient_str}")
    ...
    for step in range(args_cli.max_steps):
        ...
        if validated.any():
            ...
            for idx in v_ids:
                ...
                reach_orient_errs.append(orient_err[idx].item())
        ...
        # Periodic logging (with orient info)
        if (step + 1) % 200 == 0:
            height = env.robot.data.root_pos_w[:, 2].mean().item()
            avg_dist = dist.mean().item()
            avg_orient = orient_err.mean().item()
            ...
            print(f"  [{step+1:5d}/{args_cli.max_steps}] "
                  ...
                  f"EE={avg_dist:.3f} OrErr={avg_orient:.2f} "
                  f"H={height:.3f} Falls={falls}")
    ...
    avg_reach_orient = np.mean(reach_orient_errs) if reach_orient_errs else 0
    height_final = env.robot.data.root_pos_w[:, 2].mean().item()

    print(f"\n{'='*80}")
    print(f"STAGE 3 PLAY RESULTS -- {mode_str}")
    print(f"{'='*80}")
    ...
    print(f"  Avg Reach Orient Error: {avg_reach_orient:.3f} rad ({np.degrees(avg_reach_orient):.1f} deg)")
    print(f"  Falls: {falls}")
    print(f"  Final Height: {height_final:.3f}m")
    if total_steps > 0:
        print(f"  Reach Rate: {validated_reaches / (total_steps / 1000):.1f} per 1K steps")
    print(f"  Checkpoint: {args_cli.checkpoint}")
    print(f"  Level: {ckpt_level}, Orient: {use_orient} (thresh={orient_thresh:.1f})")
    print(f"{'='*80}")
