"""
DEX3 Grasp Training — Phase B (Frozen Loco + Arm, Finger-Only Training)
========================================================================
Robot is free-standing (fix_root_link=False). Frozen loco policy keeps
the robot upright, frozen arm policy reaches toward the object. Only 7
right-hand finger joints are trained via PPO.

ARCHITECTURE:
  FrozenLocoActor (66->15) [512,256,128]+LN+ELU — frozen, keeps robot standing
  FrozenArmActor  (39->7)  [256,256,128]+ELU    — frozen, reaches toward object
  GraspActor      (45->7)  [256,128,64]+ELU     — trained, finger delta position
  GraspCritic     (45->1)  [256,128,64]+ELU     — trained, value estimation

CURRICULUM (5 levels, all shapes):
  L0: noise=0.00 (object at default pos)
  L1: noise=0.01
  L2: noise=0.02
  L3: noise=0.03
  L4: noise=0.04 (FINAL)
...
USAGE:
  isaaclab.bat -p .../train_grasp_phase_b.py --num_envs 2048 --headless \
      --loco_checkpoint logs/ulc/.../model_best.pt \
      --arm_checkpoint logs/ulc/.../model_best.pt
  isaaclab.bat -p .../train_grasp_phase_b.py --num_envs 64  # smoke test (no frozen policies)

Author: Turan (isaac-g1-ulc), 2026-04-04
"""
...
ARM_FROZEN_ACTION_SCALE = 2.0  # residual scale for frozen arm actor
...
def parse_args():
    parser = argparse.ArgumentParser(description="DEX3 Grasp Phase B Training")
    parser.add_argument("--num_envs", type=int, default=2048)
    parser.add_argument("--max_iterations", type=int, default=30000)
    parser.add_argument("--checkpoint", type=str, default=None,
                        help="Resume from checkpoint")
    parser.add_argument("--loco_checkpoint", type=str, default=None,
                        help="Stage 2 Loco checkpoint for frozen locomotion")
    parser.add_argument("--arm_checkpoint", type=str, default=None,
                        help="Stage 2 Arm checkpoint for frozen reaching (can be same as loco)")
    parser.add_argument("--experiment_name", type=str, default="g1_grasp_phase_b")
...
def quat_to_euler_xyz(q):
    """Convert wxyz quaternion to euler xyz angles."""
...
# ============================================================================
print("=" * 80)
print("GRASP PHASE B: Frozen Loco + Arm, Finger-Only Training")
...
class FrozenLocoActor(nn.Module):
    """Frozen loco actor from Stage 2 Loco (66 obs -> 15 act, [512,256,128]+LN+ELU)."""
    def __init__(self):
        super().__init__()
        layers = []
        prev = 66
        for h in [512, 256, 128]:
            ...
            prev = h
        layers.append(nn.Linear(prev, 15))
        self.net = nn.Sequential(*layers)
        self.log_std = nn.Parameter(torch.zeros(15))
...
class FrozenArmActor(nn.Module):
    """Frozen arm actor from Stage 2 Loco (39 obs -> 7 act, [256,256,128]+ELU)."""
    def __init__(self):
        super().__init__()
        layers = []
        prev = 39
        for h in [256, 256, 128]:
            layers += [nn.Linear(prev, h), nn.ELU()]
            prev = h
        layers.append(nn.Linear(prev, 7))
        self.net = nn.Sequential(*layers)
...
def create_env(num_envs, device):
    ...
    class GraspPhaseBEnv(DirectRLEnv):
        def __init__(self, cfg, **kw):
            super().__init__(cfg, **kw)
            jn = self.robot.joint_names
            print(f"\n[GraspB] Robot joints ({len(jn)}): {jn}")
            ...
            # Loco joint indices (legs + waist = 15)
            self.loco_idx = []
            ...
            print(f"  Loco joints: {len(self.loco_idx)}")
            ...
            print(f"  Left arm joints: {len(self.left_arm_idx)}")

            # Default poses for frozen policies
            ...
            # Frozen policies (loaded from main() after env creation)
            self.loco_actor = None  # Set by main() via load_frozen_policies
            self.arm_actor = None
            self.prev_loco_act = torch.zeros(self.num_envs, 15, device=self.device)
            self.prev_arm_act = torch.zeros(self.num_envs, 7, device=self.device)
            ...
            self.arm_target_orient[:, 2] = -1.0  # palm down
            ...
            print(f"\n[GraspB] {self.num_envs} envs, Level {self.curr_level}")
        ...
        def _sample_object_pos(self, env_ids):
            ...
            # Place object on table, 25-35cm in front of robot (arm reach distance)
            root_pos = self.robot.data.root_pos_w[env_ids]
            obj_pos = root_pos.clone()
            obj_pos[:, 0] += 0.30  # 30cm forward (+X direction)
            obj_pos[:, 1] -= 0.15  # slightly to the right (right arm)
            obj_pos[:, 2] = 0.75   # table surface height
            if noise > 0:
        ...
        # ================================================================
        # FROZEN POLICY OBS BUILDERS
        # ================================================================

        def _get_loco_obs(self):
            """Build 66-dim loco observation (V6.2 format)."""
            r = self.robot
            ...
            # Zero velocity commands (standing)
            vel_cmd = torch.zeros(self.num_envs, 3, device=self.device)
            height_cmd = torch.full((self.num_envs, 1), HEIGHT_DEFAULT, device=self.device)
            torso_cmd = torch.zeros(self.num_envs, 3, device=self.device)

            # Gait phase (standing = 0)
            gait_phase = torch.zeros(self.num_envs, 2, device=self.device)
        ...
        def _get_arm_obs(self):
            """Build 39-dim arm observation (Stage 2 Loco format)."""
            r = self.robot
            q = r.data.root_quat_w
            root_pos = r.data.root_pos_w

            arm_pos = r.data.joint_pos[:, self.right_arm_idx]
            arm_vel = r.data.joint_vel[:, self.right_arm_idx]

            # EE position (palm) in body frame
            palm_pos_w = r.data.body_pos_w[:, self.palm_idx]
            palm_quat_w = r.data.body_quat_w[:, self.palm_idx]
            ee_body = quat_apply_inverse(q, palm_pos_w - root_pos)

            # Target: object position in body frame
            obj_pos, _, _ = self._get_active_obj_state()
            target_body = quat_apply_inverse(q, obj_pos - root_pos)

            # Position error
            pos_error = target_body - ee_body

            # Orient error (palm down target)
            orient_err = torch.ones(self.num_envs, 1, device=self.device) * 0.5  # approximate

            # Steps since spawn (normalized)
            steps_norm = torch.ones(self.num_envs, 1, device=self.device) * 0.5
        ...
        def _reset_idx(self, env_ids):
            ...
            self.prev_loco_act[env_ids] = 0.0
            self.prev_arm_act[env_ids] = 0.0
        ...
        def _apply_action(self):
            # 1. Loco inference (frozen, deterministic) -- keeps robot standing
            if self.loco_actor is not None:
                with torch.no_grad():
                    loco_obs = self._get_loco_obs()
                    loco_act = self.loco_actor(loco_obs).clamp(-1.0, 1.0)
                    self.prev_loco_act = loco_act.clone()
            else:
                loco_act = torch.zeros(self.num_envs, 15, device=self.device)

            # 2. Arm inference (frozen, deterministic) -- reaches toward object
            if self.arm_actor is not None:
                with torch.no_grad():
                    arm_obs = self._get_arm_obs()
                    arm_act = self.arm_actor(arm_obs).clamp(-1.5, 1.5)
                    self.prev_arm_act = arm_act.clone()
            else:
                arm_act = torch.zeros(self.num_envs, 7, device=self.device)

            # 3. Finger control (trained, delta accumulation)
            ...
            # 4. Build full joint target
            target = self.default_joint_pos.unsqueeze(0).expand(self.num_envs, -1).clone()

            # Loco: direct position (default + scale * action)
            loco_scales = torch.cat([
                torch.full((12,), LEG_ACTION_SCALE, device=self.device),
                torch.full((3,), WAIST_ACTION_SCALE, device=self.device),
            ])
            target[:, self.loco_idx] = self.default_loco.unsqueeze(0) + loco_scales.unsqueeze(0) * loco_act

            # Right arm: residual (default + scale * action)
            target[:, self.right_arm_idx] = self.default_right_arm.unsqueeze(0) + ARM_FROZEN_ACTION_SCALE * arm_act

            # Left arm: default
            target[:, self.left_arm_idx] = self.default_left_arm.unsqueeze(0)

            # Fingers: trained target (right hand)
            target[:, self.finger_idx] = self.finger_target

            # Left hand fingers: default
            left_finger_idx = []
            for name in HAND_JOINT_NAMES:
                if 'left' in name:
                    jn = self.robot.joint_names
                    if name in jn:
                        left_finger_idx.append(jn.index(name))
            if left_finger_idx:
                left_finger_idx_t = torch.tensor(left_finger_idx, device=self.device)
                target[:, left_finger_idx_t] = self.default_hand[:len(left_finger_idx)].unsqueeze(0)
...
def main():
    ...
    if args_cli.loco_checkpoint:
        print(f"\n[Frozen Loco] Loading: {args_cli.loco_checkpoint}")
        loco_actor = FrozenLocoActor().to(device)
        loco_ckpt = torch.load(args_cli.loco_checkpoint, map_location=device, weights_only=False)
        loco_state = loco_ckpt.get("model", loco_ckpt)
        loaded = 0
        for key, val in loco_state.items():
            # Map loco_actor.X or actor.X -> net.X
            if key.startswith("loco_actor."):
                our_key = "net." + key[len("loco_actor."):]
            elif key.startswith("actor."):
                our_key = "net." + key[len("actor."):]
            elif key == "loco_log_std" or key == "log_std":
                our_key = "log_std"
            else:
                continue
            if our_key in loco_actor.state_dict() and loco_actor.state_dict()[our_key].shape == val.shape:
                loco_actor.state_dict()[our_key].copy_(val)
                loaded += 1
        loco_actor.eval()
        for p in loco_actor.parameters():
            p.requires_grad = False
        env.loco_actor = loco_actor
        print(f"  Loaded {loaded} params, FROZEN")

    # Load frozen arm policy
    if args_cli.arm_checkpoint:
        print(f"\n[Frozen Arm] Loading: {args_cli.arm_checkpoint}")
        arm_actor = FrozenArmActor().to(device)
        ...
        for key, val in arm_state.items():
            if key.startswith("arm_actor.net."):
                our_key = "net." + key[len("arm_actor.net."):]
            elif key.startswith("arm_actor.") and key != "arm_actor.log_std":
                our_key = "net." + key[len("arm_actor."):]
            else:
                continue
            if our_key in arm_actor.state_dict() and arm_actor.state_dict()[our_key].shape == val.shape:
                arm_actor.state_dict()[our_key].copy_(val)
                loaded += 1
        arm_actor.eval()
        for p in arm_actor.parameters():
            p.requires_grad = False
        env.arm_actor = arm_actor
        print(f"  Loaded {loaded} params, FROZEN")
    ...
    print(f"\n{'='*80}")
    print("STARTING GRASP PHASE B TRAINING")
    print(f"  Grasp: {OBS_DIM} obs -> {ACT_DIM} act")
