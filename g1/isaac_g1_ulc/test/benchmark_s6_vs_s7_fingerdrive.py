...
# ===========================================================================
# VARIANT OF benchmark_s6_vs_s7.py -- DO NOT EDIT THE ORIGINAL TO MATCH THIS.
#
# The original pins every finger joint open for every policy. S6u, however, was
# trained driving its own fingers from the policy output for 18,838 of its 20,000
# iterations (train/23dof/train_ulc_stage_6_unified_asrun_c545e8a.py L1486-1488).
# This copy can run S6u in the configuration it trained in, so that the mismatch
# can be measured instead of estimated. S6s and S7 behave exactly as in the
# original: they have no finger outputs to drive.
#
# It also records the mean absolute arm action at every step, for every policy
# and mode, to action_magnitude.csv -- the original only printed it for the first
# five steps and never saved it.
# ===========================================================================

parser = argparse.ArgumentParser(
    description="S6 vs S7 Benchmark - finger-driving variant")
...
parser.add_argument("--s6u_fingers", type=str, default="both",
                    choices=["pin", "policy", "both"],
                    help="S6u finger command source: pin (as in the original benchmark), policy (as in training), or both as two separate columns "
                         "(default: both)")
...
class DualACStage6Unified(nn.Module):
    ...
    def get_actions(self, loco_obs, arm_obs, deterministic=True, full_arm=False):
        loco_mean = self.loco_actor(loco_obs)
        arm_out = self.arm_actor(arm_obs)  # [N, 12]
        # full_arm=True returns all 12 (5 arm + 7 finger), i.e. what the policy
        # actually commanded during training. full_arm=False reproduces the
        # original benchmark, which discards the finger outputs.
        arm_mean = arm_out if full_arm else arm_out[:, :5]
        if deterministic:
            return loco_mean, arm_mean
        loco_std = self.loco_actor.log_std.clamp(-2, 1).exp()
...
class BenchmarkEnv(DirectRLEnv):
    ...
    def __init__(self, cfg, render_mode=None, **kwargs):
        ...
        # Off unless a session turns it on; S6s and S7 never do.
        self.drive_fingers = False
    ...
    def _pre_physics_step(self, actions):
        """Apply actions: leg (12) + arm (5) [+ finger (7) when driven]."""
        ...
        if self.drive_fingers and actions.shape[-1] >= 24:
            # Identical to the as-run trainer, c545e8a L1486-1488: no clipping,
            # [-1, 1] mapped onto the joint range.
            finger_actions = actions[:, 17:24]
            finger_normalized = (finger_actions + 1.0) / 2.0
            target_pos[:, self.finger_idx] = (
                self.finger_lower
                + finger_normalized * (self.finger_upper - self.finger_lower)
            )
        else:
...
def run_policy_session(env, net, policy_name, get_arm_obs_fn,
                       ...
                       full_arm=False, drive_fingers=False, mag_rows=None):
    """Run one policy for N steps and collect per-target metrics.

    full_arm      -- ask the net for all 12 arm outputs (S6u only).
    drive_fingers -- send the 7 finger outputs to the robot (S6u only).
    mag_rows      -- list that collects per-step action magnitudes.
    """
    env.drive_fingers = bool(drive_fingers)
    device = env.device
    per_target = []
    target_id = 0
    ...
    with torch.no_grad():
        for step in range(steps):
            ...
            # Get actions
            if full_arm:
                leg_act, arm_full = net.get_actions(
                    loco_obs, arm_obs, deterministic=True, full_arm=True
                )
                # Pinned variant: identical to the original, which slices to 5.
                arm_act = arm_full if drive_fingers else arm_full[:, :5]
            else:
                leg_act, arm_act = net.get_actions(
                    loco_obs, arm_obs, deterministic=True
                )
                arm_full = None

            if mag_rows is not None:
                _src = arm_full if arm_full is not None else arm_act
                _abs = _src.abs()
                _arm5 = _abs[:, :5].mean().item()
                if _abs.shape[-1] >= 12:
                    _arm12 = _abs.mean().item()
                    _fing = _abs[:, 5:12].mean().item()
                else:
                    _arm12 = ""
                    _fing = ""
                mag_rows.append([
                    policy_name, mode_name, step,
                    "%.6f" % _arm5,
                    "" if _arm12 == "" else "%.6f" % _arm12,
                    "" if _fing == "" else "%.6f" % _fing,
                ])
...
def save_action_magnitude(mag_rows, output_dir):
    """Write every step's mean absolute arm action, plus per-session means."""
    path = os.path.join(output_dir, "action_magnitude.csv")
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["policy", "mode", "step", "arm5_abs_mean",
                    "arm12_abs_mean", "finger7_abs_mean"])
        w.writerows(mag_rows)
    print("[OUTPUT] action_magnitude.csv -> %s (%d rows)" % (path, len(mag_rows)))

    agg = {}
    for policy, mode, step, a5, a12, f7 in mag_rows:
        key = policy + ' | ' + mode
        d = agg.setdefault(key, {"n": 0, "arm5": 0.0, "arm12": 0.0,
                                 "finger7": 0.0, "n12": 0})
        d["n"] += 1
        d["arm5"] += float(a5)
        if a12 != "":
    ...
    for key, d in agg.items():
        rec = {
            "steps": d["n"],
            "arm5_abs_mean_over_session": round(d["arm5"] / d["n"], 6),
        }
        if d["n12"]:
            rec["arm12_abs_mean_over_session"] = round(d["arm12"] / d["n12"], 6)
            rec["finger7_abs_mean_over_session"] = round(d["finger7"] / d["n12"], 6)
        out[key] = rec
    jpath = os.path.join(output_dir, "action_magnitude_summary.json")
    with open(jpath, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2)
    print("[OUTPUT] action_magnitude_summary.json -> %s" % jpath)
...
def main():
    ...
    # Entry: (net, label, obs_fn, pos_th, orient_th, full_arm, drive_fingers)
    policies = []
    if s6u_net is not None:
        # S6 unified: uses object_rel_ee (zeros) at index 46-48, NOT target_orient
        if args.s6u_fingers in ("pin", "both"):
            # Bit-identical to the original benchmark.
            policies.append((s6u_net, "S6u_pinned", env.get_arm_obs_s6u,
                             0.04, 1.0, True, False))
        if args.s6u_fingers in ("policy", "both"):
            # The configuration S6u actually trained in.
            policies.append((s6u_net, "S6u_fingerdrive", env.get_arm_obs_s6u,
                             0.04, 1.0, True, True))
    if s6s_net is not None:
        # S6 simplified: uses target_orient at index 46-48 (same as S7)
        policies.append((s6s_net, "S6s_simplified", env.get_arm_obs_s6,
                         0.04, 1.0, False, False))
    policies.append((s7_net, "S7_antigaming", env.get_arm_obs_s7,
                     0.04, 2.0, False, False))
    ...
    for mode_name in modes:
        ...
        for (net, label, obs_fn, obs_pos_th, obs_orient_th,
             full_arm, drive_fingers) in policies:
            torch.manual_seed(args.seed)
            np.random.seed(args.seed)
            env.full_reset()
            print("[FINGERS] %s | %s -> %s" % (
                label, mode_name, "policy-driven" if drive_fingers else "pinned open"))
            result = run_policy_session(
                env, net, label, obs_fn,
                obs_pos_th, obs_orient_th, args.steps, mode_name,
                full_arm=full_arm, drive_fingers=drive_fingers, mag_rows=mag_rows
            )
            all_results.append(result)
    env.drive_fingers = False

    # ---- Save outputs ----
    save_per_target_csv(all_results, output_dir)
    save_summary_json(all_results, output_dir)
    save_action_magnitude(mag_rows, output_dir)
